...
function getTopMatchID(data) {
	if (data.size === 0) return Promise.reject(new Error("not posted on HN"));
	return Promise.resolve(
		data.results.filter((a) => a.parent_id == null)[0].objectID
	);
}
...
export default class App extends Component {
	constructor(props) {
		...
		if (this.props.id) {
			fetchCommentsByID(this.props.id).then((comments) => {
	...
	render() {
		...
		return (
			<div className={style["hn-app"]}>
				<Header title={this.state.comments.title} id={this.state.comments.id} />
				<Home
